...
APPBASE="build/macos-aarch64/DalorianScape.app"
...
build() {
    ...
    cp native/build-aarch64/src/DalorianScape $APPBASE/Contents/MacOS/
...
dmg() {
    ...
    if ! hdiutil imageinfo DalorianScape-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo DalorianScape-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit DalorianScape-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple DalorianScape-aarch64.dmg
    fi
}
