...
APPBASE="build/macos-x64/DalorianScape.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/DalorianScape.jar
    ...
    cp native/build-x64/src/DalorianScape $APPBASE/Contents/MacOS/
    cp build/libs/DalorianScape.jar $APPBASE/Contents/Resources/
    ...
    otool -l $APPBASE/Contents/MacOS/DalorianScape
}
...
dmg() {
    ...
    mv DalorianScape\ *.dmg DalorianScape-x64.dmg

    # dump for CI
    hdiutil imageinfo DalorianScape-x64.dmg

    if ! hdiutil imageinfo DalorianScape-x64.dmg | grep -q "Format: UDBZ" ; then
        echo "Format of resulting dmg was not UDBZ, make sure your create-dmg has support for --format"
        exit 1
    fi

    if ! hdiutil imageinfo DalorianScape-x64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit DalorianScape-x64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple DalorianScape-x64.dmg
    fi
}
